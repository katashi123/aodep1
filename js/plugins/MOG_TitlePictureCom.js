 ...
 * É necessário ter o arquivo Title.png na pasta img/titles/
 * @default true
 * 
...
var $title_step = 0; //タイトル画面遷移管理

//16/09/05 タイトル画面タッチによるメニュー表示処理追加
...
Window_TitleCommand.prototype.processOk = function() {
	if($title_step != 0){ // メニューが表示されてから操作可
		//console.log("Window_TitleCommand processOK");
	...
	if($title_step != 0){ // メニューが表示されてから操作可
	    if (this.isCursorMovable()) {
	        ...
	        if (this.index() !== lastIndex) {
	        	//console.log("Window_TitleCommand processCursorMove");
...
Scene_Title.prototype.createCursorCommand = function() {
    ...
    this._cursor = new Sprite(ImageManager.loadTitle1("Cursor"));
...
Scene_Title.prototype.create_picture_commands = function() {
	...
	for (i = 0; i < this._commandWindow._list.length; i++){
    	this._com_pictures.push(ImageManager.loadTitle1("Command_" + i));
	...
	if($title_step == 0){
		this._com_pictures_start = ImageManager.loadTitle1("pushscreen");
		this._com_sprites_start = new Sprite(this._com_pictures_start);
		this._com_sprites_start.x = 255;
		this._com_sprites_start.y = 450;
		this._count = 0;
		//this._com_sprites_start.alpha = 0;
		this.addChild(this._com_sprites_start);
	}
};
...
Scene_Title.prototype.refresh_picture_command = function() {
	...
	for (i = 0; i < this._com_sprites.length; i++){
		...
		if(!Utils.isMobileDevice()){
			...
			if(i == 0){
				this._com_sprites[i].y = this.cpsx[1] + 10;
			}else if(i == 1){
				this._com_sprites[i].y = this.cpsx[1] + 30;
			}else{
				this._com_sprites[i].y = this.cpsx[1];
			}
		}
...
Scene_Title.prototype.update_picture_commands = function() {
	...
	if($title_step == 0){
		...
		var tmp_num = Math.sin((Math.PI*2)/60*this._count );
		//console.log("update_picture_commands count = " + this._count + ": tmp_num :" + tmp_num);
		this._com_sprites_start.alpha = tmp_num;
		//this._com_sprites_start.alpha = 2;
		this._count ++;
		
		if (TouchInput.isTriggered()) {
			SoundManager.playOk();
			$title_step = 1;
			this._com_sprites_start.visible = false;
			for (i = 0; i < this._commandWindow._list.length; i++){
